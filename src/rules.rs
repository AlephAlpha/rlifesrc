...
use crate::life::Life;
use crate::nt_life::NTLife;

pub fn parse_life(string: &str) -> Result<Life, String> {
    ...
    let mut life = (one_of("Bb".chars()),
        ...
        optional(token('/')),
        one_of("Ss".chars()),
        numbers).map(|(_, b, _, _, s)| Life::new(b, s));
...
pub fn parse_nt_life(string: &str) -> Result<NTLife, String> {
    ...
    let chars_to_number: Vec<_> = nbhd_maps.iter()
        .enumerate()
        .map(|(i, m)| {
            let keys = m.keys().cloned();
            let chars = choice((
                many1(one_of(keys.clone())),
                token('-').with(many(one_of(keys.clone())))
                    .map(move |s: Vec<_>| m.keys().cloned()
                        .filter(|c| !s.contains(c))
                        .collect()),
                value(keys.collect())));
            token(i.to_string().pop().unwrap())
                .with(chars.map(move |v: Vec<_>| v.iter()
    ...
    let numbers = many(choice((
        ...
        chars_to_number[1].clone(),
        chars_to_number[2].clone(),
        chars_to_number[3].clone(),
        chars_to_number[4].clone(),
        chars_to_number[5].clone(),
        chars_to_number[6].clone(),
        chars_to_number[7].clone(),
